...
from mcp.server.fastmcp import Context, FastMCPServer, RequestError
...
class TerminalSession:
    ...
    def execute(
        ...
        timeout: SmartTimeout = SmartTimeout(),
    ) -> Dict[str, Any]:
        """
...
def _reload_permissions(signum, frame):
    permissions.reload()
...
def _execute_internal(
    ...
    if dangerous:
        raise RequestError(f"Blocked: {dangerous}")

    def stream_cb(line: str) -> None:
        if ctx:
            try:
                ctx.event_stream.info(line.rstrip("\r\n"))
            except Exception:
                pass

    result = session.execute(command, stream_callback=stream_cb, timeout=SmartTimeout())
...
async def execute_command(command: str, ctx: Context) -> Dict[str, Any]:
    ...
    if not allowed:
        if category == "always_ask":
            raise RequestError(f"Permission required: {message}")
        raise RequestError(message)
    return _execute_internal(command, ctx, is_override=False)
...
async def execute_with_override(
    ...
    category = permissions.classify(base)
    if category == "always_block":
        raise RequestError("Cannot override interactive/dangerous commands")
    if category != "always_ask":
        raise RequestError(f"Command '{base}' does not require override")
    if len(safety_override_reason) < 50:
        raise RequestError("Provide a detailed reason (50+ chars)")
    if not accept_risk:
        raise RequestError("Must set accept_risk=True to proceed")
    ...
    result = _execute_internal(command, ctx, is_override=True)
...
async def user_approve_command(
    ...
    if user_confirmation != EXACT:
        raise RequestError(f"Confirmation must be exactly: '{EXACT}'")
    base = _base_cmd(command)
    if duration == "session":
    ...
    raise RequestError("duration must be 'session' or 'permanent'")
